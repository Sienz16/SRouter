...
- A standalone React hook whose public name is `use<Name>` belongs in `apps/web/src/hooks/<Name>.ts`. Use `.tsx` only when the hook file contains JSX.
- Examples: `useTheme` → `apps/web/src/hooks/Theme.ts`; `useMobile` → `apps/web/src/hooks/Mobile.ts`.
