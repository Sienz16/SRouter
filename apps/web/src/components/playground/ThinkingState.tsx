...
> = {
    ...
    Search: {
        ...
        query: "best AI router architecture",
        rows: [
            {
    ...
    Coding: {
        ...
        rows: [
            { primary: "Read", secondary: "usePlayground.ts", mono: true },
            {
                primary: "Edit",
...
const TONES = ["bg-[var(--ink)]", "bg-amber-500", "bg-emerald-500"];
...
export default function ThinkingState({
    ...
    const baseV = VARIANTS[variant] ?? VARIANTS.Steps;

    // Adapt to live thinkingText / duration if provided
    const dynamicRows: ThinkingRow[] =
        customRows ??
        (thinkingText
            ? thinkingText
                  .split("\n")
                  .filter((l) => l.trim().length > 0)
                  .map((l) => ({ primary: l.trim() }))
            : baseV.rows);

    const durationSec = durationMs ? (durationMs / 1000).toFixed(1) : "4";
    const dynamicDone = doneLabel ?? (durationMs ? `Thought for ${durationSec}s` : baseV.done);
    const dynamicActive = activeLabel ?? (modelId ? `Thinking with ${modelId}` : baseV.active);
    ...
    const working = isStreaming !== undefined ? isStreaming : stage < 3;
    const visible =
    ...
    }, [visible, expanded, variant, stage, dynamicRows]);

    return (
        <div key={variant} className="my-1.5 flex w-full flex-col font-mono text-xs">
            {/* header — shared across variants */}
            <button
                type="button"
                aria-expanded={expanded}
                onClick={() => setManualExpanded((current) => !(current ?? autoExpanded))}
                className="-mx-1.5 flex w-fit items-center gap-2 rounded-[8px] px-1.5 py-1 transition-colors duration-100 hover:bg-[var(--hover)] cursor-pointer"
            >
                <svg
                    width="16"
                    height="16"
                ...
                {working ? (
                    ...
                    <span
                        className="text-[12.5px] font-medium whitespace-nowrap text-[var(--ink-2)]"
                                    ...
                                    <>
                                        ...
                                        <span
                                            className={`min-w-0 truncate text-[12px] ${
                                                variant === "Reasoning"
                                                    ? "whitespace-normal leading-relaxed text-[var(--ink-2)]"
                                        ...
                                        {row.secondary && (
                                            <span
                                                className={`shrink-0 text-[11px] text-[var(--ink-3)] ${
                                                    row.mono ? "font-mono" : ""
                                                }`}
                                            >
                                                {row.secondary}
                                        ...
                                        {row.add !== undefined && (
                                            <span className="shrink-0 font-mono text-[10.5px] tabular-nums">
                                                <span className="text-emerald-500">+{row.add}</span>{" "}
                                                <span className="text-rose-500">−{row.del}</span>
                                            </span>
                                        )}
                                    </>
                                );
